(function($) {
	...
	$(window).load(function() {
		var modal = $('#wbcr-factory-feedback-101-deactivate-form');
		...
		modal.click(function(evt) {
			...
			if( $target.hasClass('wbcr-factory-feedback-101-modal-body') || $target.hasClass('wbcr-factory-feedback-101-modal-footer') ) {
				return;
			}
			...
			if(
				...
				($target.parents('.wbcr-factory-feedback-101-modal-body').length > 0 || $target.parents('.wbcr-factory-feedback-101-modal-footer').length > 0)
			) {
				return;
			}
		...
		modal.on('click', '.wbcr-factory-feedback-101-modal-footer .button', function(evt) {
			evt.preventDefault();
			if( $(this).hasClass('disabled') ) {
				return;
			}

			var _parent = $(this).parents('.wbcr-factory-feedback-101-modal:first');
			var _this = $(this);
			...
			if( _this.hasClass('allow-deactivate') ) {
				...
				if( $radio.val() ) {
					$.ajax({
						...
						method: 'POST',
						data: {
							action: 'wbcr-factory-feedback-101-save',
							plugin: plugin_slug,
							reason_id: $radio.val(),
							reason_more: userReason,
							anonymous: modal.find('#wbcr-factory-feedback-101-anonymous-checkbox').is(':checked')
							           ? 1
							           : 0,
							_wpnonce: modal.data('nonce')
						},
						beforeSend: function() {
							_parent.find('.wbcr-factory-feedback-101-modal-footer .button').addClass('disabled');
							_parent.find('.wbcr-factory-feedback-101-modal-footer .button-secondary').text('Processing...');
						},
						error: function() {
